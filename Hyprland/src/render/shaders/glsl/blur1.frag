...
void main() {
    ...
    // compute HSL once
    vec3 hslOrig = rgb2hsl(color.rgb);
    vec3 hsl = hslOrig;

    // apply per-pass saturation and brightness multipliers
    hsl.y = clamp(hsl.y * passSaturation, 0.0, 1.0);
    hsl.z = clamp(hsl.z * passBrightness, 0.0, 1.0);
    ...
    if (vibrancy == 0.0) {
        ...
        float boostBase = hsl2[1] > 0.0 ? smoothstep(b1 - c * 0.5, b1 + c * 0.5, 1.0 - (pow(1.0 - hsl2[1] * cos(a), 2.0) + pow(1.0 - perceivedBrightness * sin(a), 2.0))) : 0.0;

        float saturation = clamp(hslOrig[1] + (boostBase * vibrancy) / float(passes), 0.0, 1.0);

        // apply per-pass saturation multiplier after vibrancy boost
        saturation = clamp(saturation * passSaturation, 0.0, 1.0);

        // apply per-pass brightness (lum) already applied above in hsl.z
        vec3  newColor = hsl2rgb(vec3(hsl[0], saturation, hsl.z));
