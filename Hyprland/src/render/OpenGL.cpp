...
CFramebuffer* CHyprOpenGLImpl::blurFramebufferWithDamage(float a, CRegion* originalDamage, CFramebuffer& source) {
    ...
    auto drawPass = [&](WP<CShader> shader, ePreparedFragmentShader frag, CRegion* pDamage, int passIndex) {
        ...
        // per-pass brightness and saturation multipliers
        const float passBrightness = powf(0.9f, float(passIndex)); // reduce brightness by 10% per pass
        const float passSaturation = powf(1.1f, float(passIndex)); // increase saturation per pass
        shader->setUniformFloat(SHADER_PASS_BRIGHTNESS, passBrightness);
        shader->setUniformFloat(SHADER_PASS_SATURATION, passSaturation);
    ...
    for (auto i = 1; i <= BLUR_PASSES; ++i) {
        ...
        drawPass(shader, SH_FRAG_BLUR1, &tempDamage, i); // down
    }
    ...
    for (auto i = BLUR_PASSES - 1; i >= 0; --i) {
        ...
        drawPass(shader, SH_FRAG_BLUR2, &tempDamage, i);     // up
    }
...
void CHyprOpenGLImpl::renderRoundedShadow(const CBox& box, int round, float roundingPower, int range, const CHyprColor& color, float a) {
    ...
    shader->setUniformMatrix3fv(SHADER_PROJ, 1, GL_TRUE, glMatrix.getMatrix());
    // Neon purple glow: #7b5ccf -> (123,92,207)
    const float neonR = 123.0f / 255.0f;
    const float neonG = 92.0f / 255.0f;
    const float neonB = 207.0f / 255.0f;

    // pulsate intensity using global timer
    const float t = m_globalTimer.getSeconds();
    const float pulse = 0.75f + 0.25f * std::sinf(t * 2.0f); // ranges ~[0.5,1.0]

    const float neonA = a * pulse; // respect passed-in alpha multiplier

    shader->setUniformFloat4(SHADER_COLOR, neonR, neonG, neonB, neonA);
    ...
    // use additive blending for neon glow, restore after drawing
    ...
    // restore premultiplied alpha blend func used elsewhere
